...
namespace :stimulus do
  ...
  namespace :manifest do
    task display: :environment do
      puts Stimulus::Manifest.generate_from(Rails.root.join(Stimulus.controllers_path))
    end

    task update: :environment do
      manifest =
        Stimulus::Manifest.generate_from(Rails.root.join(Stimulus.controllers_path))

      File.open(Rails.root.join("#{Stimulus.controllers_path}index.js"), "w+") do |index|
